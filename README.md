...
- **`meshserver-api.md`**：**（遗留）** meshserver 中心化群的本地 API 说明。新服务端能力请使用 **meshchat-server**（HTTP，见同工作区 `meshchat-server/docs/API.md`）；meshserver 路径仅兼容旧部署。
...
meshproxy 现在内置了一套**点对点加密聊天**原型页面，入口为 `http://127.0.0.1:19080/chat`。这套聊天能力不是独立再造一张网络，而是直接复用当前 meshproxy 已有的去中心化网络底座：
