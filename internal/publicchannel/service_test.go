...
func TestRPCListChannelMessagesAlias(t *testing.T) {
	t.Parallel()
	...
	ownerSvc := mustTestService(t, ctx, ownerHost, ownerPS, ownerPriv, filepath.Join(t.TempDir(), "owner-rpc-alias.db"))
	readerSvc := mustTestService(t, ctx, readerHost, readerPS, readerPriv, filepath.Join(t.TempDir(), "reader-rpc-alias.db"))

	summary, err := ownerSvc.CreateChannel(CreateChannelInput{Name: "rpc-alias"})
	if err != nil {
		t.Fatal(err)
	}
	channelID := summary.Profile.ChannelID
	if _, err := ownerSvc.CreateChannelMessage(channelID, UpsertMessageInput{Text: "first"}); err != nil {
		t.Fatal(err)
	}

	var resp GetMessagesResponse
	if err := readerSvc.rpcCall(ctx, ownerPeerID, "list_channel_messages", struct {
		ChannelID string `json:"channel_id"`
		Limit     int    `json:"limit"`
	}{ChannelID: channelID, Limit: 10}, &resp); err != nil {
		t.Fatal(err)
	}
	if len(resp.Items) != 1 {
		t.Fatalf("want 1 message from alias rpc, got %d", len(resp.Items))
	}
	if resp.Items[0].Content.Text != "first" {
		t.Fatalf("want alias rpc message text first, got %q", resp.Items[0].Content.Text)
	}
}
